...
exports.useCreateRereplyMutation = exports.useCreateReplyMutation = void 0;
...
function useCreateReplyMutation(commentId) {
    ...
    var create = client_1.useMutation(graphql_1.CREATE_REPLY_MUTATION, {
        variables: values,
        update: function (proxy, result) {
            ...
            if (result.data) {
                data.getComment = __assign(__assign({}, data.getComment), { replys: __spreadArrays([
                        result.data.createReply.id
                    ], data.getComment.replys), replyCount: data.getComment.replyCount + 1 });
...
exports.useCreateReplyMutation = useCreateReplyMutation;
function useCreateRereplyMutation(replyId) {
    var _a = react_1.useState(false), IsModalOpen = _a[0], setIsModalOpen = _a[1];
    var _b = react_1.useState({
        replyId: replyId,
        body: initialValue
    ...
    var create = client_1.useMutation(graphql_1.CREATE_REREPLY_MUTATION, {
        variables: values,
        update: function (proxy, result) {
            var data = __assign({}, proxy.readQuery({
            ...
            if (result.data) {
                data.getReply = __assign(__assign({}, data.getReply), { rereplys: __spreadArrays([
                        result.data.createRereply.id
                    ], data.getReply.rereplys), rereplyCount: data.getReply.rereplyCount + 1 });
...
exports.useCreateRereplyMutation = useCreateRereplyMutation;
