...
export default function Comment({
    ...
    const comment = useGetCommentQuery(id);

    /// commentUser
    const { user: commentUser, setUser: setCommentUser } = useGetUserQuery();

    useEffect(() => {
        if (comment) setCommentUser(comment.user);
    }, [comment, setCommentUser]);

    /// upvote | downvote
    const { upvoteComment, downvoteComment } = useVoteCommentMutation(id);
    ...
    const {
        ...
        editCallback,
        values,
    } = useEditCommentMutation(id, comment);

    const deleteComment = useDeleteCommentMutation(id, answerId);
